...
In class, we defined the grid using latitude and longitude coordinates, resulting in a grid of squares covering the geographical area of interest.

```{r}
cell_deg <- 0.02  # grid resolution

# bounding box of observed points (in lon/lat)
bbox_ll <- st_bbox(
  st_as_sf(pts_local, coords = c("lon", "lat"), crs = 4326)
)

# make regular grid in lon/lat
grid_ll <- st_make_grid(
  st_as_sfc(bbox_ll),
  cellsize = c(cell_deg, cell_deg),
  square = TRUE
)

grid_sf <- st_sf(geometry = grid_ll) |>
  st_transform(3857)
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "Original Regular Grid (cell_deg = 0.02)",
    subtitle = "Fixed spatial discretization overlaid with sample trajectories"
  ) +
  guides(color = "none")
```
